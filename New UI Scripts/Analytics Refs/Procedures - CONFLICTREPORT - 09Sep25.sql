...
CREATE OR REPLACE FUNCTION CONFLICTREPORT.PUBLIC.GETFULLSHIFTTIME("BILLABLEMINUTESFULLSHIFT" NUMBER(38,0), "SHIFT1_START" TIMESTAMP_NTZ(9), "SHIFT1_END" TIMESTAMP_NTZ(9))
RETURNS NUMBER(38,4)
LANGUAGE SQL
AS '
    SELECT 
        CASE 
            ...
            WHEN SHIFT1_START IS NOT NULL AND SHIFT1_END IS NOT NULL THEN TIMESTAMPDIFF(MINUTE, SHIFT1_START, SHIFT1_END)
            ELSE 0
        END
';

CREATE OR REPLACE FUNCTION CONFLICTREPORT.PUBLIC.GETOVERLAPTIME("BILLABLEMINUTESOVERLAP" NUMBER(38,0), "SHIFT1_START" TIMESTAMP_NTZ(9), "SHIFT1_END" TIMESTAMP_NTZ(9), "SHIFT2_START" TIMESTAMP_NTZ(9), "SHIFT2_END" TIMESTAMP_NTZ(9))
RETURNS NUMBER(38,0)
LANGUAGE SQL
AS '
    SELECT 
        CASE 
            WHEN billableMinutesOverlap IS NOT NULL THEN billableMinutesOverlap
            WHEN LEAST(shift1_end, shift2_end) > GREATEST(shift1_start, shift2_start)
            THEN TIMESTAMPDIFF(
                MINUTE,
                GREATEST(shift1_start, shift2_start),
                LEAST(shift1_end, shift2_end)
            )
            ELSE 0
        END
';
