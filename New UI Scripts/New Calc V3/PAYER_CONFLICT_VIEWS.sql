...
CREATE OR REPLACE VIEW CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_BASE AS
...
SELECT
    *
FROM CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_BASE
WHERE "Row_Number" = 1
  AND CONTYPE IS NOT NULL;

-- Aggregated view: rollups for dashboard KPIs
CREATE OR REPLACE VIEW CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_AGGREGATED_CONTYPE AS
SELECT 
...
FROM CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_BASE
WHERE "Row_Number" = 1 AND CONTYPE IS NOT NULL
GROUP BY 
    PROCESSING_PAYERID,
