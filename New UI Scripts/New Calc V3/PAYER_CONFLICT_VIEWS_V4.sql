-- CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_AGGREGATED_COMMON source

create or replace view CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_AGGREGATED_COMMON(
	ID,
	"Contract",
	"ConContract",
	"PayerID",
...
-- CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_AGGREGATED_COMMON_TEMP source

create or replace view CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_AGGREGATED_COMMON_TEMP(
	ID,
	"Contract",
	"ConContract",
	"PayerID",
...
WITH SOURCE AS (
    SELECT *
    FROM CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_COMMON_TEMP -- Reading from the temp view
    WHERE PAYER_ACTIVE
),
DEDUP AS (
    SELECT
            ...
            PARTITION BY
                ...
                CASE
                    ...
                    ELSE
                        "AppVisitID" || '|' || "ConAppVisitID"
                END
            ORDER BY "CONFLICTID" DESC, "ID" ASC
        ) AS rn
    FROM SOURCE
)
SELECT
    ID, "Contract", "ConContract", "PayerID", "ConPayerID", "ProviderID", "ProviderName", SSN, CRDATEUNIQUE, "GroupID",
    CONFLICTID, "AppVisitID", "ConAppVisitID", "VisitID", "ConVisitID", "ShVTSTTime", "ShVTENTime", "CShVTSTTime",
    "CShVTENTime", "BilledRateMinute", "StatusFlag", "Billed", "VisitStartTime", "ServiceCode", HAS_TIME_OVERLAP,
    HAS_TIME_DISTANCE, HAS_IN_SERVICE, COUNTY, CONTYPE, CONTYPEOLD, FULL_SHIFT_AMOUNT, OVERLAP_AMOUNT, COSTTYPE,
    VISITTYPE, PAYER_ACTIVE, CONPAYER_ACTIVE, "CaregiverID", "AppCaregiverID", "VisitDate",
    "AideCode", "AideFName", "AideLName", "AideSSN", G_CRDATEUNIQUE, "FlagForReview", "PA_PatientID", "PA_PName", rn
FROM DEDUP;


-- CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_COMMON source

create or replace view CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_COMMON(
	ID,
	"Contract",
	"ConContract",
	"PayerID",
...
create or replace view CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_LIST(
	ID,
	"GroupID",
	SSN,
	"ProviderID",
...
-- CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_LIST_TEMP source

create or replace view CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_LIST_TEMP(
	ID,
	"GroupID",
	SSN,
	"CaregiverID",
...
SELECT
    "ID", "GroupID", "SSN", "CaregiverID", "AppCaregiverID", "VisitID", "AppVisitID", "VisitDate", "AideCode",
    "AideFName", "AideLName", COALESCE("AideSSN", "SSN") AS "AideSSN", "G_CRDATEUNIQUE" AS "CRDATEUNIQUE",
    "PayerID", "Contract", CONTYPEOLD, "FlagForReview", "PA_PatientID", "PA_PName", RN
FROM CONFLICTREPORT_SANDBOX.PUBLIC.V_PAYER_CONFLICTS_AGGREGATED_COMMON_TEMP -- Reading from the temp view
WHERE RN = 1
ORDER BY "GroupID" DESC;