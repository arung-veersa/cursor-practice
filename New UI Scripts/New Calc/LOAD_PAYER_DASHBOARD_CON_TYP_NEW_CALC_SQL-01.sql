    ...
    FROM (
        ...
        SELECT 
            ...
            ROW_NUMBER() OVER (
                ...
                ORDER BY 
                    CASE 
                        WHEN V1."PayerID" = V1."PayerID" THEN 1  -- Primary payer gets priority
                        ELSE 2  -- Conflicting payer gets lower priority
                    END
            ) as rn
        FROM CONFLICTREPORT_SANDBOX.PUBLIC.CONFLICTVISITMAPS AS V1
        INNER JOIN CONFLICTREPORT_SANDBOX.PUBLIC.CONFLICTS AS V2 
        ...
        SELECT 
            ...
            ROW_NUMBER() OVER (
                ...
                ORDER BY 
                    CASE 
                        WHEN V1."ConPayerID" = V1."ConPayerID" THEN 1  -- Primary payer gets priority
                        ELSE 2  -- Conflicting payer gets lower priority
                    END
            ) as rn
        FROM CONFLICTREPORT_SANDBOX.PUBLIC.CONFLICTVISITMAPS AS V1
