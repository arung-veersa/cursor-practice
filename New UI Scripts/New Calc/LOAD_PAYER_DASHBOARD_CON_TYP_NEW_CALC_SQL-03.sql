...
WITH PAYER_LIST AS (
    -- Get all distinct PayerIds from active, non-demo payers
...
PAYER_RECORDS AS (
    ...
    -- Create records for ConPayerID when ConPayerID is active (excluding self-conflicts)
    SELECT 
        *,
    ...
    WHERE V1."ConPayerID" IN (SELECT APID FROM PAYER_LIST)
        AND V1."ConPayerID" != V1."PayerID"  -- Exclude self-conflicts from priority 2
),
