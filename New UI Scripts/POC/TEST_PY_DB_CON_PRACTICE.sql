            ...
            var insercontypes = `
                ...
                FROM (
                    SELECT
                        ...
                        CASE 
                            WHEN a."SameSchTimeFlag" = ''Y'' THEN ''Exact Schedule Time Match''
                            WHEN a."SameVisitTimeFlag" = ''Y'' THEN ''Exact Visit Time Match''
                            WHEN a."SchAndVisitTimeSameFlag" = ''Y'' THEN ''Exact Schedule and Visit Time Match''
                            WHEN a."SchOverAnotherSchTimeFlag" = ''Y'' THEN ''Schedule time overlap''
                            WHEN a."VisitTimeOverAnotherVisitTimeFlag" = ''Y'' THEN ''Visit Time Overlap''
                            WHEN a."SchTimeOverVisitTimeFlag" = ''Y'' THEN ''Schedule and Visit time overlap''
                            WHEN a."DistanceFlag" = ''Y'' THEN ''Time- Distance''
                            WHEN a."InServiceFlag" = ''Y'' THEN ''In-Service''
                            WHEN a."PTOFlag" = ''Y'' THEN ''PTO''
                        END AS CONTYPEDESC,
                        CASE 
                            WHEN a."SameSchTimeFlag" = ''Y'' THEN ''1''
                            WHEN a."SameVisitTimeFlag" = ''Y'' THEN ''2''
                            WHEN a."SchAndVisitTimeSameFlag" = ''Y'' THEN ''3''
                            WHEN a."SchOverAnotherSchTimeFlag" = ''Y'' THEN ''4''
                            WHEN a."VisitTimeOverAnotherVisitTimeFlag" = ''Y'' THEN ''5''
                            WHEN a."SchTimeOverVisitTimeFlag" = ''Y'' THEN ''6''
