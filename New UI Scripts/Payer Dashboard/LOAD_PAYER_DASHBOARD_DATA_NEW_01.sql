...
AS '
	try {
	    ...
	    while (payerResult.next()) {
			...
			var insercontypes = `
				INSERT INTO CONFLICTREPORT_SANDBOX.PUBLIC.PAYER_DASHBOARD_CON_TYP_NEW(PAYERID, CRDATEUNIQUE, CONTYPE, CONTYPES, STATUSFLAG, COSTTYPE, VISITTYPE, CO_TO, CO_SP, CO_OP, CO_FP)
				SELECT
				*
				FROM
					(
					SELECT
						...
						CASE WHEN a."Billed" = ''yes'' THEN ''Recovery'' ELSE ''Avoidance'' END AS "COSTTYPE",
						CASE WHEN a."VisitStartTime" IS NULL THEN ''Scheduled'' WHEN a."VisitStartTime" IS NOT NULL AND a."Billed" != ''yes'' THEN ''Confirmed'' WHEN a."VisitStartTime" IS NOT NULL AND a."Billed" = ''yes'' THEN ''Billed'' END AS "VISITTYPE",
						...
						SUM( CASE WHEN a.APID = ''${payerId}'' AND a."BilledRateMinute" > 0 AND a."BILLABLEMINUTESOVERLAP" IS NOT NULL THEN a."BILLABLEMINUTESOVERLAP" * a."BilledRateMinute" WHEN a.APID = ''${payerId}'' AND a."BilledRateMinute" > 0 AND b."ShVTSTTime" >= a."ShVTSTTime" AND b."ShVTSTTime" <= a."ShVTENTime" AND b."ShVTENTime" > a."ShVTENTime" THEN TIMESTAMPDIFF(MINUTE, b."ShVTSTTime", a."ShVTENTime") * a."BilledRateMinute" WHEN a.APID = ''${payerId}'' AND a."BilledRateMinute" > 0 AND a."ShVTSTTime" >= b."ShVTSTTime" AND a."ShVTSTTime" <= b."ShVTENTime" AND a."ShVTENTime" > b."ShVTENTime" THEN TIMESTAMPDIFF(MINUTE, a."ShVTSTTime", b."ShVTENTime") * a."BilledRateMinute" WHEN a.APID = ''${payerId}'' AND a."BilledRateMinute" > 0 AND b."ShVTSTTime" >= a."ShVTSTTime" AND b."ShVTENTime" <= a."ShVTENTime" THEN TIMESTAMPDIFF(MINUTE, b."ShVTSTTime", b."ShVTENTime") * a."BilledRateMinute" WHEN a.APID = ''${payerId}'' AND a."BilledRateMinute" > 0 AND b."ShVTSTTime" < a."ShVTSTTime" AND b."ShVTENTime" > a."ShVTENTime" THEN TIMESTAMPDIFF(MINUTE, a."ShVTSTTime", a."ShVTENTime") * a."BilledRateMinute" WHEN a.APID = ''${payerId}'' AND a."BilledRateMinute" > 0 AND b."ShVTSTTime" < b."ShVTSTTime" AND b."ShVTENTime" > b."ShVTENTime" THEN TIMESTAMPDIFF(MINUTE, b."ShVTSTTime", b."ShVTENTime") * a."BilledRateMinute" ELSE 0 END ) AS "OverlapPrice",
					...
					LEFT JOIN (
						...
						a.CONFLICTID <> b.CONFLICTID
						AND a."GroupID" = b."GroupID"
						GROUP BY a.CRDATEUNIQUE, a."StatusFlag", CASE WHEN a."Billed" = ''yes'' THEN ''Recovery'' ELSE ''Avoidance'' END, CASE WHEN a."VisitStartTime" IS NULL THEN ''Scheduled'' WHEN a."VisitStartTime" IS NOT NULL AND a."Billed" != ''yes'' THEN ''Confirmed'' WHEN a."VisitStartTime" IS NOT NULL AND a."Billed" = ''yes'' THEN ''Billed'' END
				UNION ALL
					SELECT
						''${payerId}'' AS PAYERID,
						a."CRDATEUNIQUE" AS "CRDATEUNIQUE",
