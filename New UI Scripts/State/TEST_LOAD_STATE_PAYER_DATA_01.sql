CREATE OR REPLACE PROCEDURE CONFLICTREPORT_SANDBOX.PUBLIC.TEST_LOAD_STATE_PAYER_DATA_01()
RETURNS VARCHAR
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS '
	try {
		...
		var truncate1Stmt = snowflake.createStatement({
			sqlText: `TRUNCATE TABLE CONFLICTREPORT_SANDBOX.PUBLIC.TEST_STATE_PAYER_01`
		});
		...
		var payerStmt = snowflake.createStatement({
			sqlText: `
				...
				WHERE P."Is Active" = TRUE 
					AND P."Is Demo" = FALSE AND P."Payer State" = ''NY''
			`
		});
		...
		while (payerResult.next()) {
			...
			//-------------------------PAYER CON TYPE---------------------
			var insercontypes = `
				INSERT INTO CONFLICTREPORT_SANDBOX.PUBLIC.TEST_STATE_PAYER_01(
					PAYERID, PROVIDERID, CRDATEUNIQUE, CONTYPE, CONTYPES, STATUSFLAG, 
					COSTTYPE, VISITTYPE, COUNTY, CO_TO, CO_SP, CO_OP, CO_FP
				)
				SELECT *
				FROM (
