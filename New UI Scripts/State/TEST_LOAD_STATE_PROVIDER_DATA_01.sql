CREATE OR REPLACE PROCEDURE CONFLICTREPORT_SANDBOX.PUBLIC.TEST_LOAD_STATE_PROVIDER_DATA_01()
RETURNS VARCHAR
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS '
	
	var SQL3 = `TRUNCATE TABLE CONFLICTREPORT_SANDBOX.PUBLIC.TEST_STATE_PROVIDER_01`;
	var SQL4 = `INSERT INTO CONFLICTREPORT_SANDBOX.PUBLIC.TEST_STATE_PROVIDER_01 (PAYERID, PROVIDERID, CRDATEUNIQUE, STATUSFLAG, COSTTYPE, VISITTYPE, COUNTY, TO_TO, TO_SP, TO_OP, TO_FP, TD_TO, TD_SP, TD_OP, TD_FP, IN_TO, IN_SP, IN_OP, IN_FP)
	SELECT
	...
	LEFT JOIN (
		...
		WHERE CVM."PayerState" = ''NY''
			AND NOT (CVM."PTOFlag" = ''Y'' 
		...
		GROUP BY CVM."PayerID", TO_CHAR(CVM."CRDATEUNIQUE", ''YYYY-MM-DD''), CVM."ProviderID", CVM."StatusFlag", CASE WHEN CVM."Billed" != ''yes'' THEN ''Avoidance'' ELSE ''Recovery'' END, CASE WHEN CVM."VisitStartTime" IS NULL THEN ''Scheduled'' WHEN CVM."Billed" != ''yes'' THEN ''Confirmed'' ELSE ''Billed'' END, COALESCE(CVM."P_PCounty", CVM."PA_PCounty")
	`;
